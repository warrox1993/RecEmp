...
export class CandidatureListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  supprimerCandidature(candidature: Candidature, event: MouseEvent): void {
    ...
    const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      width: '380px', data: { title: 'Confirmer suppression', message: `Supprimer la candidature pour ${candidature.entreprise} ?`, confirmText: 'Supprimer', cancelText: 'Annuler', color: 'warn' } as ConfirmDialogData
    });
  ...
  exporterEnCSV(): void {
    ...
    if (contenuCSV === "Aucune donnée à exporter.") {
